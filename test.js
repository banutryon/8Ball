...
// VARIABLES
// set array length to variable
const seedArrLength = dataTest.length;

// print length of array of objects
// console.log(`The magic 8 ball can display ${seedArrLength} messages.`);

const getRandomPhrase = (arrLen, theArray) => {
    // get random index
    const randomIndex = Math.floor(Math.random() * arrLen);
    // print random index selected
    // console.log(`randomIndex: ${randomIndex}`);
    let meme = theArray[randomIndex];
    return meme;
}

let theMeme = getRandomPhrase(seedArrLength, dataTest);
console.log(theMeme.name);
